node ('centos8') {
  ...
  stage('package') {
    ...
    withCredentials([usernamePassword(credentialsId: '0f11d4d1-6557-423c-b5ae-693cc87f7b4b', usernameVariable: 'GITHUB_USERNAME', passwordVariable: 'GITHUB_TOKEN')]) {
      ...
      sh 'podman build --format=docker -t site-search-data --build-arg=GITHUB_USERNAME=\${GITHUB_USERNAME} --build-arg=GITHUB_TOKEN=\${GITHUB_TOKEN}-f $WORKSPACE/project_home/SiteSearchData/dockerfiles/Dockerfile .'
    }
